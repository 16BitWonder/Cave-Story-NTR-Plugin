...
USA_TID 	= "000400000009B300"
EUR_TID		= "0004000000098900"
JPN_TID		= "0004000000179A00"
KOR_TID		= "00040000001A5500"
NAME 		= "CaveStory"
FTP_FOLDER_USA	= "./plugin/%s/" % USA_TID
FTP_FOLDER_EUR	= "./plugin/%s/" % EUR_TID
FTP_FOLDER_JPN	= "./plugin/%s/" % JPN_TID
FTP_FOLDER_KOR	= "./plugin/%s/" % KOR_TID
HOST		= "192.168.1.32"
PORT		= "5000"
...
def send():
	...
	ftp.cwd(FTP_FOLDER_USA); #Switch between USA, EUR, JPN and KOR as needed
	#Transfer file
...
if  not(os.path.exists("plugin/plugin/" + USA_TID)):
	os.makedirs("plugin/plugin/" + USA_TID);
if  not(os.path.exists("plugin/plugin/" + EUR_TID)):
	os.makedirs("plugin/plugin/" + EUR_TID);
if  not(os.path.exists("plugin/plugin/" + JPN_TID)):
	os.makedirs("plugin/plugin/" + JPN_TID);
if  not(os.path.exists("plugin/plugin/" + KOR_TID)):
	os.makedirs("plugin/plugin/" + KOR_TID);
shutil.copy2(COPYTOPATH, "plugin/plugin/" + USA_TID + "/" + NAME + "_MULTI.plg");
shutil.copy2(COPYTOPATH, "plugin/plugin/" + EUR_TID + "/" + NAME + "_MULTI.plg");
shutil.copy2(COPYTOPATH, "plugin/plugin/" + JPN_TID + "/" + NAME + "_MULTI.plg");
shutil.copy2(COPYTOPATH, "plugin/plugin/" + KOR_TID + "/" + NAME + "_MULTI.plg");
